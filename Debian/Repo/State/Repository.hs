...
import Extra.EnvPath (EnvPath, EnvPath(EnvPath), EnvRoot(EnvRoot), outsidePath)
import Extra.Except -- (HasIOException, liftIOError)
...
createLocalRepository root layout = do
  ...
  layout' <- liftIOError (computeLayout (outsidePath root)) >>= return . maybe layout Just
...
prepareLocalRepository :: (MonadIOError e m) => [Loc] -> EnvPath -> Maybe Layout -> [Release] -> m LocalRepository
...
prepareLocalRepository' :: (MonadIOError e m) => [Loc] -> EnvPath -> Maybe Layout -> m LocalRepository
...
prepareRemoteRepository :: (HasLoc e, MonadIOError e m, MonadRepos s m) => [Loc] -> VendorURI -> m RemoteRepository
...
loadRemoteRepository ::
    (HasLoc e, MonadIOError e m, MonadRepos s m)
    => [Loc] -> VendorURI -> m RemoteRepository
loadRemoteRepository locs uri =
    do releaseInfo <- withError (withLoc $here) $ liftIOError $ unsafeInterleaveIO $ getReleaseInfoRemote ($here : locs) uri
...
foldRepository :: (HasLoc e, MonadIOError e m, MonadRepos s m) => [Loc] -> (LocalRepository -> m a) -> (RemoteRepository -> m a) -> RepoKey -> m a
